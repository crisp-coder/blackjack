...
class PlayerWinHandler : public MessageHandler{
  public:
    PlayerWinHandler(std::shared_ptr<MessageSender> sender,
        std::shared_ptr<StateMachine> state_machine,
        std::shared_ptr<Bank> bank){
        setName("PlayerWinHandler");
    ...
    void handle(Message msg){
      ...
      if(Utils::isInteger(msg.getBody())){
        ...
        logger.LOG(bjlogger::ERROR, "Player::PlayerWinHandler: bad integer.");
        winnings = 0;
      }
      bank_->add(winnings);
      std::string balance = std::to_string(bank_->getBalance());
      std::string display_message = "Player Wins! Won " + std::to_string(winnings) + "!\n";
