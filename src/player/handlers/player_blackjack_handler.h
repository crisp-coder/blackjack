...
class PlayerBlackjackHandler : public MessageHandler{
  public:
    PlayerBlackjackHandler(std::shared_ptr<MessageSender> sender,
        std::shared_ptr<StateMachine> state_machine,
        std::shared_ptr<Bank> bank){
        setName("PlayerBlackjackHandler");
    ...
    void handle(Message msg){
      ...
      if(Utils::isInteger(msg.getBody())){
        ...
        logger.LOG(bjlogger::ERROR, "Player::PlayerBlackjackHandler: bad integer.");
        winnings = 0;
      }
      bank_->add(winnings);
      std::string balance = std::to_string(bank_->getBalance());

      std::string display_message = "Player has Blackjack! Won " + std::to_string(winnings) + "!\n";
