...
class PlayerPushHandler : public MessageHandler{
  public:
    PlayerPushHandler(std::shared_ptr<MessageSender> sender,
        std::shared_ptr<StateMachine> state_machine,
        std::shared_ptr<Bank> bank){
        setName("PlayerPushHandler");
    ...
    void handle(Message msg){
      ...
      if(Utils::isInteger(msg.getBody())){
        ...
        logger.LOG(bjlogger::ERROR, "Player::PlayerPushHandler: bad integer.");
        winnings = 0;
      }
      bank_->add(winnings);
      std::string balance = std::to_string(bank_->getBalance());
      std::string display_message = "Player pushed. Returned " + std::to_string(winnings) + "!\n";
